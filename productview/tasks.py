...
def parse_products_from_file(upload_id):
    ...
    async_to_sync(channel_layer.group_send)(channel_name, {
        'type': 'upload_message', "message": "Saving Products to Database", "saved_records": records_saved, "status": "inprogress"})
    Product.objects.filter().delete() #deleting all eisting records
...
@app.task(bind=True, autoretry_for=(requests.exceptions.RequestException,), retry_kwargs={'max_retries': 3, 'countdown': 300})
